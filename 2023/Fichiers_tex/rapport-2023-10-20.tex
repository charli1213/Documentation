...
\begin{tikzpicture}[scale = 0.8]
  ...
  \foreach \x in {1,2,3,4,5}{
   ...
   \draw (-0.5,\x-0.5) node {\x};}
\end{tikzpicture}
\caption{\label{org0d0c255}Stencil de redistribution de la masse. À gauche (a), transfert de masse horizontal vu du haut. Au milieu (b), le même transfert de masse vu en coupe verticale. À droite (c), cas tangeant au mur.}
\end{figure}

En somme, on crée une sous-routine qui peut vérifier l'épaisseur des couches produire un transfert de masse horizontal, comme illustré à la figure \ref{org0d0c255}.
...
\end{tikzpicture}
\end{center}
\caption{\label{orgdc8c700}« Stencil » utilisé pour obtenir le champs aux plus grandes échelles. À gauche (a), «stencil» pour une interoplation à ratio impair, à droite (b), «stencil» pour une interpolation à ratio pair.}
\end{figure}
