...
\caption{\label{fig:org2d38e2f}Diagnostique de l'épaisseur d'une run précise du modèle \emph{shallow water}. On peut voir sur ce zoom que l'instabilité dans l'épaisseur apparait dans le haut à droite de tous les quadrant avant que le modèle s'arrête. De gauche à droite les temps -4, -3 et en bas : -2, -1.}
\end{figure}

\subsection{Solutions : Filtre de Robert plus fort? -- \textit{<2023-10-12 Thu>}}
\label{sec:org217fac7}
...
Les valeurs utilisées sont illustrées dans le tableau \ref{tab:orgc8d292d} et l'on y retrouve aussi le nombre de pas de temps que le modèle s'est rendu avant de lâcher à cause d'une instabilité numérique causant une épaisseur nulle. 

\begin{table}[htbp]
\caption{\label{tab:orgc8d292d}Résumé des valeurs utilisées comme filtre de Robert, dans le but de « smoother » les instabilités numériques.}
\centering
\begin{tabular}{ccccc}
\hline
...
Un résumé des expériences se retrouve dans le tableau \ref{tab:orge97ad20}.

\begin{table}[htbp]
\caption{\label{tab:orge97ad20}Résumé des expériences lancées avec un pas de temps de 200 secondes au lieu de 300.}
\centering
\begin{tabular}{cccc}
\hline
...
\end{tikzpicture}
\end{center}
\caption{\label{org9d2c53e}Illustration conceptuelle de la rampe pour éviter le \emph{spin up} du modèle de vagues.}
\end{wrapfigure}
...
\end{tikzpicture}
\end{center}
\caption{\label{org06eda71}« Stencil » utilisé pour obtenir le champs aux plus grandes échelles.}
\end{figure}
