...
\end{tikzpicture}
\end{center}
\caption{\label{orge8214d2}Illustration d'un modèle \textit{shallow water} à quatre couches (\(n_k = 4\)).}
\end{wrapfigure}
...
En ordre décroissant, il est donc possible de créer une méthode itérative qui appelle le \emph{RHS} du \emph{eta} couche inférieure (\(nk+1\)) à l'aide de la dérivée de l'équation \ref{eq:org2e0ce94}, soit
\begin{align}
...
\textbf{N.B. 2)} La formulation \ref{eq:org9cdc7d3} est toujours valides, car c'est la définition de la QGPV à plusieurs couches.
