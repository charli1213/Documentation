...
Les descriptions proviennent du code source du modèle, de la documentation de Wavewatch III et de la litérature scientifique ( par exemple, voir \Citet{ardhuin2010semiempirical}, \citep{couvelard2020development} et \citet*{wu_breivik_2019}).
...
Dans la litérature, il est extrêmement clair que les quantités physiques nommées \emph{wave-supported stress} (\(\boldsymbol{\tau}_{IN}\)) et \emph{wave to ocean momentum flux} (\(\boldsymbol{\tau}_{DS}\)) représentent une contrainte de cisaillement ou un stress (voir \Citet{breivik_al_2015}, \Citet{ardhuin2010semiempirical} et \Citet{couvelard2020development} en exemple).
...
\end{equation}
Ces derniers s'appuient principalement sur \Citet{bidlot2012present}, \citeauthor[voir][]{janssen_1989} (\citeyear{janssen_1989} et \citeyear{janssen_1991}).

\subsubsection{Janssen, 1989}
\label{sec:orge40b5ab}

Dans un premier temps, \citet*{janssen_1989} décrit le stress du vent avec des unitées de \(N\cdot m^{-2}\).
...
\end{equation}
et c'est ce que l'article de \Citet{bidlot2012present} semble indiquer à l'aide d'un \emph{air-sea density ratio} (\(\varepsilon\)).


\subsubsection{Janssen, 1991}
\label{sec:org7cce1ad}
...
\Citet[voir eq. 6 de l'article]{bidlot2012present} décrit ce tansfert de momentum à l'aide de l'expression
\begin{equation}
